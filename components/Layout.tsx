...
export const Layout: React.FC<LayoutProps> = ({ children }) => {
  ...
  const [unreadMessages, setUnreadMessages] = useState<
    { id: string; senderName: string; content: string; senderId: string }[]
  >([]);
  ...
  const [uiStyle, setUiStyle] = React.useState<"classic" | "glass">(
    () => (localStorage.getItem("uiStyle") as any) || "classic",
  );
  ...
  const pendingRevisionsCount = React.useMemo(() => {
    ...
    return Object.values(profile.revisions).filter(
      (r) =>
        ...
        <div className="flex items-center justify-between w-full">
          ...
          {badge !== undefined &&
            ...
            (isBadgeDot ? (
              ...
              <span
                className={`px-2 py-0.5 rounded-full text-xs font-bold ${isActive ? "bg-white text-primary-600" : "bg-red-500 text-white group-hover:bg-red-600"}`}
              >
                {badge}
              </span>
            ))}
    ...
    <div className="min-h-screen flex bg-gray-50 dark:bg-dark-surface text-slate-900 dark:text-slate-100 transition-colors duration-200">
      ...
      <div className="fixed top-0 w-full z-20 bg-white dark:bg-dark-card glass-panel border-b border-gray-200 dark:border-dark-border px-4 py-3 flex items-center justify-between shadow-sm h-16">
        ...
        <div className="flex items-center space-x-3">
          ...
          <Link
            to="/account"
            className="hidden sm:flex items-center space-x-2 text-sm font-medium text-slate-600 dark:text-slate-300 hover:text-primary-600 transition-colors"
          >
          ...
          <button
            onClick={() =>
              setUiStyle(uiStyle === "classic" ? "glass" : "classic")
            }
            ...
            {showNotifs && (
              <div className="absolute right-0 mt-2 w-80 bg-white dark:bg-dark-card rounded-xl shadow-2xl border border-gray-200 dark:border-dark-border overflow-hidden z-50 animate-in fade-in slide-in-from-top-2">
                ...
                <Link
                  to="/inbox"
                  onClick={() => setShowNotifs(false)}
                  className="block p-3 text-center text-xs font-bold text-primary-600 hover:bg-gray-50 dark:hover:bg-dark-surface dark:text-primary-400 transition-colors border-t border-gray-100 dark:border-dark-border"
                >
                  View All Messages
                </Link>
              </div>
          ...
          <button
            onClick={toggleTheme}
            className="p-2 rounded-full hover:bg-gray-100 dark:hover:bg-dark-border text-slate-500 dark:text-slate-400 transition-colors"
          >
          ...
          <div className="p-4 border-t border-gray-200 dark:border-dark-border space-y-4 bg-gray-50 dark:bg-dark-surface/50">
            ...
            >
              ...
              <div className="overflow-hidden flex-1">
                <p className="text-sm font-bold truncate text-slate-800 dark:text-slate-100 group-hover:text-primary-600 dark:group-hover:text-primary-400 transition-colors">
                  {profile?.displayName || "User"}
                </p>
      ...
      {/* Main Content */}
      <main
        className={`flex-1 min-w-0 scroll-smooth flex flex-col pt-16 ${location.pathname.includes("/sql/problem/") ? "h-screen overflow-hidden" : "overflow-y-auto min-h-screen"}`}
      >
        <div
          className={`w-full ${location.pathname.includes("/sql/problem/") ? "flex-1 overflow-hidden" : "max-w-[1600px] mx-auto p-4 sm:p-6 lg:p-10"}`}
        >
          {children}
        </div>
      </main>

      {!location.pathname.includes("/sql/problem/") && <FocusTimer />}
      <GlobalSearch />
    </div>
  );
};
