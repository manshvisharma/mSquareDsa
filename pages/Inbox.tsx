    ...
    useEffect(() => {
        if (selectedUser) {
            ...
            messages.filter(m => m.senderId === selectedUser && m.receiverId === user?.uid && !m.read).forEach(m => {
                setDoc(doc(db, COLLECTIONS.MESSAGES, m.id), { read: true }, { merge: true });
            });
        }
                        ...
                        conversations.map(c => (
                            ...
                            >
                                {c.partnerAvatar && !c.partnerAvatar.includes('dicebear') ? (
                                    <img src={c.partnerAvatar} alt={c.partnerName} className="w-12 h-12 flex-shrink-0 rounded-full border border-primary-200 object-cover shadow-sm bg-white" />
                                ) : (
                                    <div className="w-12 h-12 flex-shrink-0 rounded-full bg-gradient-to-tr from-primary-400 to-purple-500 text-white flex items-center justify-center font-bold text-lg shadow-sm border border-primary-200 overflow-hidden relative">
                                ...
                                {c.unread > 0 && <div className="w-2.5 h-2.5 bg-red-500 rounded-full shadow-sm shadow-red-400/50"></div>}
                            </button>
                        ))
                    )}
                </div>
                ...
                {selectedUser ? (
                    <>
                        <div className="p-4 border-b border-gray-100 dark:border-dark-border flex items-center gap-3 bg-gray-50 dark:bg-dark-surface/50 rounded-t-2xl">
                            ...
                            <h3 className="font-bold text-lg text-slate-800 dark:text-white hover:text-primary-600 cursor-pointer flex items-center gap-1">
                        ...
                        <div className="flex-1 overflow-y-auto p-4 space-y-4 custom-scrollbar">
                            {messages.filter(m => m.senderId === selectedUser || m.receiverId === selectedUser).map(msg => {
                                ...
                                return (
                                    <div key={msg.id} className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
                                        <div className={`group relative max-w-[75%] rounded-2xl px-4 py-2 shadow-sm ${isMe ? 'bg-primary-600 text-white rounded-br-sm' : 'bg-gray-100 dark:bg-dark-surface text-slate-800 dark:text-gray-100 rounded-bl-sm border border-gray-200 dark:border-dark-border'}`}>
                                            ...
                                            <span className={`text-[10px] mt-1 block ${isMe ? 'text-primary-200' : 'text-gray-400'}`}>
                                                {new Date(msg.timestamp).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}
                                            </span>
                                            {isMe && (
                                              <button onClick={() => setMsgToDelete(msg.id)} className="absolute top-2 -left-8 p-1 text-gray-400 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-opacity">
                                                  <Trash2 size={14} />
                                              </button>
                                            )}
                                        </div>
