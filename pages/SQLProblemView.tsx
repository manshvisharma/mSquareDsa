...
export default function SQLProblemView() {
    ...
    const [inputTables, setInputTables] = useState<{tableName: string, fields: string[], rows: any[]}[] | null>(null);
    ...
    useEffect(() => {
        const fetchInputTables = async () => {
            ...
            try {
                ...
                const tablesRes = await pg.query("SELECT table_name FROM information_schema.tables WHERE table_schema='public'");
        ...
        const fetchProblem = async () => {
            ...
            try {
                ...
                if (!snap.empty) {
                    ...
                    setTestCasesSql(data.sampleTestCases && data.sampleTestCases.length > 0 ? data.sampleTestCases : [data.visibleSetupSql || '']);
                }
            } catch(e) {
                console.error(e);
            } finally {
    ...
    useEffect(() => {
        if (slug && queryCode !== undefined && problem && queryCode !== problem.starterQuery) {
    ...
    const compareRows = (actual: any[], expected: any[], expectedFields: string[], actualFields: string[]) => {
    ...
    const countVerdict = (results: TestCaseResult[]): 'Accepted' | 'Wrong Answer' | 'Error' => {
        if (results.some(r => r.error)) return 'Error';
        if (results.some(r => !r.passed)) return 'Wrong Answer';
        return 'Accepted';
    };

    const [leftPanelTab, setLeftPanelTab] = useState<'description' | 'submissions' | 'result'>('description');
    ...
    const runQuery = async (isSubmit = false) => {
        ...
        try {
            ...
            const runTestCase = async (i: number, isHiddenCase: boolean) => {
                ...
                try {
                    ...
                    if (problem.solutionQuery) {
                        ...
                        if (problem.validationQuery) {
                            ...
                            expectedRes = { fields: vExp.fields.map(f => f.name), rows: vExp.rows };
                        } else {
                    ...
                    if (problem.validationQuery) {
                        ...
                        userRes = { fields: vUser.fields.map(f => f.name), rows: vUser.rows };
                    } else {
                    ...
                    if (expectedRes && userRes) {
                        passed = compareRows(userRes.rows, expectedRes.rows, expectedRes.fields, userRes.fields);
                    } else if (!expectedRes && userRes) {
                        if (i > 0) {
                            errorMsg = "Cannot evaluate dynamic testcase automatically without a reference 'solutionQuery' assigned to this problem.";
                            passed = false;
                        } else {
                            passed = true;
                        }
                ...
                if (!isHiddenCase) {
                    return { passed, error: errorMsg, userResult: userRes, expectedResult: expectedRes, timeMs };
                } else {
                    return { passed, error: passed ? null : 'A hidden testcase failed.', userResult: null, expectedResult: null, timeMs, isHidden: true };
                }
            };
            ...
            for (let i = 0; i < casesToRun.length; i += concurrency) {
                chunks.push(casesToRun.slice(i, i + concurrency).map((_, idx) => i + idx));
            }
            ...
            for (const chunk of chunks) {
                if (shouldStop) {
                    for (const i of chunk) {
                        newResults[i] = { passed: false, error: 'Skipped due to previous failure', userResult: null, expectedResult: null, timeMs: 0, isHidden: true };
                    }
                    continue;
                }
                const promises = chunk.map(async (i) => {
                    const isHiddenCase = isSubmit && hiddenCount > 0 && i >= casesToRun.length - hiddenCount;
                ...
                setRunningProgress({ current: Math.min(completed, casesToRun.length), total: casesToRun.length });
            }
        ...
        <div className="h-full flex flex-col font-sans bg-[#F0F0F0] dark:bg-[#121212]">
            <div className="flex items-center justify-between px-4 h-12 bg-white dark:bg-[#1e1e1e] border-b border-gray-200 dark:border-[#333] shrink-0 shadow-sm z-10">
                <div className="flex items-center gap-4">
                    <button onClick={() => navigate('/sql')} className="text-gray-500 hover:text-gray-900 dark:hover:text-gray-300 transition-colors">
                        <ChevronLeft size={18} />
                    </button>
                    <h1 className="font-semibold text-sm text-slate-800 dark:text-gray-200 tracking-tight">{problem.title}</h1>
                </div>
                <div className="flex gap-3 items-center">
                    {runningProgress && (
                        <div className="flex items-center gap-2 text-xs font-medium text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-900/20 px-3 py-1.5 rounded-md mr-1">
                            ...
                            <span>{runningProgress.current}/{runningProgress.total}</span>
                        </div>
                    )}
                    <button 
                    ...
                    >
                        {(dbLoading || (isRunning && !isSubmitting)) ? <RefreshCw className="animate-spin" size={14} /> : <Play size={14} />}
                    ...
                    >
                        {(isRunning && isSubmitting) ? <RefreshCw className="animate-spin" size={14} /> : <Send size={14} />}
            ...
            <div className="flex-1 overflow-hidden p-2.5">
                <PanelGroup orientation="horizontal" className="h-full">
                    ...
                    <Panel defaultSize={40} minSize={25} className="bg-white dark:bg-[#1e1e1e] rounded-xl border border-gray-200 dark:border-[#333] flex flex-col shadow-sm">
                        <div className="flex border-b border-gray-100 dark:border-[#333] px-2 pt-2 shrink-0 items-center justify-between">
                            <div className="flex">
                                ...
                                {leftPanelTab === 'result' && (
                                    <button
                                        className="px-4 py-2 font-medium text-sm border-b-2 transition-colors border-primary-500 text-emerald-600 dark:text-emerald-400"
                                    >
                                        Result
                                    </button>
                                )}
                            ...
                            {leftPanelTab === 'description' && (
                                <>
                                    <div className="flex items-center gap-3 mb-6">
                                        <span className={`px-2.5 py-1 rounded-md text-xs font-semibold tracking-wide ${getDifficultyClass()} bg-slate-50 dark:bg-[#2d2d2d]`}>
                                            {problem.difficulty}
                                        </span>
                                        {isSolved && (
                                ...
                                <div className="space-y-4">
                                    {submissionsList.length === 0 ? (
                                        ...
                                        submissionsList.map(sub => (
                                            <div key={sub.id} className="p-4 rounded-xl border border-gray-100 dark:border-[#333] bg-slate-50 dark:bg-[#1a1a1a]">
                                                <div className="flex items-center justify-between mb-3">
                                                    <span className={`font-semibold text-sm ${sub.status === 'Accepted' ? 'text-emerald-600' : 'text-red-500'}`}>
                                                        {sub.status}
                                                    </span>
                                                ...
                                                <div className="text-xs text-slate-500 mb-2">Execution Time: {sub.executionTimeMs}ms</div>
                                                <pre className="text-xs p-3 rounded-lg overflow-x-auto bg-white dark:bg-[#121212] border border-gray-100 dark:border-[#222]">
                                                    <code className="font-mono text-slate-700 dark:text-gray-300">{sub.query}</code>
                                                </pre>
                                            </div>
                                        ))
                                    )}
                                ...
                                <div className="flex flex-col h-full overflow-y-auto">
                                    {isRunning ? (
                                        <div className="flex flex-col items-center justify-center py-20 text-slate-500">
                                            ...
                                            <h2 className="text-lg font-semibold text-slate-800 dark:text-gray-200 mb-2">Evaluating Submission</h2>
                                            <p className="text-sm">Running test case {runningProgress ? runningProgress.current : 0} of {runningProgress ? runningProgress.total : '...'}</p>
                                        </div>
                                    ) : (
                                        <div className="flex flex-col">
                                            {verdict === 'Accepted' ? (
                                                <div className="flex flex-col items-center justify-center p-8 bg-emerald-50 dark:bg-emerald-900/10 rounded-xl border border-emerald-200 dark:border-emerald-800/30 mb-6">
                                                    ...
                                                    <h2 className="text-2xl font-bold text-emerald-700 dark:text-emerald-400 mb-2">Accepted!</h2>
                                                    <p className="text-sm text-emerald-600/80 dark:text-emerald-400/80 text-center">Your query passed all test cases.</p>
                                                </div>
                                            ) : (
                                                <div className="flex flex-col items-center justify-center p-8 bg-red-50 dark:bg-red-900/10 rounded-xl border border-red-200 dark:border-red-800/30 mb-6">
                                                    ...
                                                    <h2 className="text-2xl font-bold text-red-700 dark:text-red-400 mb-2">
                                                        {verdict === 'Error' ? 'Runtime Error' : 'Wrong Answer'}
                                                    </h2>
                                                    <p className="text-sm text-red-600/80 dark:text-red-400/80 text-center">Your query failed on one or more test cases.</p>
                                                </div>
                                            )}

                                            <div className="grid gap-3">
                                                {runResults.map((res, i) => ({ res, idx: i + 1 })).filter(({res}) => !res.isHidden || !res.passed).map(({ res, idx }) => (
                                                    <div key={idx} className={`p-3 rounded-lg border ${res.passed ? 'border-gray-100 dark:border-[#333] bg-white dark:bg-[#1a1a1a]' : 'border-red-200 dark:border-red-900/50 bg-red-50 dark:bg-red-900/10'} flex flex-col gap-1`}>
                                                        <div className="flex items-center justify-between">
                                                            <div className="flex items-center gap-3">
                                                                {res.passed ? <CheckCircle2 size={18} className="text-emerald-500" /> : <XCircle size={18} className="text-red-500" />}
                                                                <span className="font-medium text-sm text-slate-700 dark:text-gray-300">Test Case {idx} {res.isHidden ? '(Hidden)' : ''}</span>
                                                            </div>
                                                            <span className="text-xs font-mono text-slate-500">{res.timeMs}ms</span>
                                                        </div>
                    ...
                    <Panel defaultSize={60} minSize={30} className="flex flex-col">
                        <PanelGroup orientation="vertical" className="h-full">
                            {/* Editor */}
                            <Panel defaultSize={60} minSize={20} className="bg-white dark:bg-[#1e1e1e] rounded-xl border border-gray-200 dark:border-[#333] flex flex-col shadow-sm overflow-hidden">
                            ...
                            {/* Console */}
                            <Panel defaultSize={40} minSize={10} className="bg-white dark:bg-[#1e1e1e] rounded-xl border border-gray-200 dark:border-[#333] flex flex-col shadow-sm">
                                <div className="flex items-center justify-between px-3 h-10 border-b border-gray-100 dark:border-[#333] bg-slate-50 dark:bg-[#252526] shrink-0">
                                    <div className="flex gap-2 h-full pt-1.5">
                                        ...
                                        >
                                            Test Results
                                            {verdict === 'Accepted' && <div className="w-2 h-2 rounded-full bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.8)]"/>}
                                            {verdict === 'Wrong Answer' && <div className="w-2 h-2 rounded-full bg-red-500 shadow-[0_0_8px_rgba(239,68,68,0.8)]"/>}
                                            {verdict === 'Error' && <div className="w-2 h-2 rounded-full bg-amber-500 shadow-[0_0_8px_rgba(245,158,11,0.8)]"/>}
                                        </button>
                                    </div>
                                </div>
                                <div className="flex-1 overflow-auto flex flex-col relative text-sm">
                                    ...
                                    <div className="flex px-4 py-2 gap-2 overflow-x-auto shrink-0 border-b border-gray-100 dark:border-[#333] bg-white dark:bg-[#1e1e1e]">
                                         {(consoleTab === 'output' && runResults.length > 0 ? runResults : testCasesSql).filter((_, idx) => {
                                             if (consoleTab === 'output') return !runResults[idx]?.isHidden;
                                             return true;
                                         }).map((_, idx) => {
                                             return (
                                            ...
                                            <div className="h-full flex flex-col max-w-4xl space-y-6">
                                                ...
                                                {inputTables ? (
                                                    inputTables.length > 0 ? (
                                                                            ...
                                                                            <thead className="bg-slate-50 dark:bg-[#252525] border-b border-gray-200 dark:border-[#333]">
                                                                                <tr>
                                                                                    {table.fields.map((f, j) => (
                                                                                        <th key={j} className="px-4 py-2.5 font-semibold text-slate-700 dark:text-gray-300">{f}</th>
                                                                                    ))}
                                                                                </tr>
                                                                            </thead>
                                                                            <tbody className="divide-y divide-gray-100 dark:divide-[#333]">
                                                                                {table.rows.map((r, rowIdx) => (
                                                                                    <tr key={rowIdx} className="hover:bg-slate-50/80 dark:hover:bg-[#2a2a2a]">
                                                                                        {table.fields.map((f, colIdx) => (
                                                                                            <td key={colIdx} className="px-4 py-2.5 text-slate-600 dark:text-gray-400 font-mono">{String(r[f])}</td>
                                                                                        ))}
                                                                                    </tr>
                                                                                ))}
                                                                                {table.rows.length === 0 && (
                                                                                    <tr>
                                                                                        <td colSpan={table.fields.length || 1} className="px-4 py-4 text-center text-slate-400 italic font-mono bg-slate-50/50 dark:bg-[#1a1a1a]">Empty</td>
                                                                                    </tr>
                                                                                )}
                                                                            </tbody>
                                                                        </table>
                                                        ...
                                                        <div className="text-sm text-slate-500 italic">No tables created in this testcase.</div>
                                                    )
                                                ) : (
                                                    <div className="text-sm text-slate-500 italic">Loading input tables...</div>
                                                )}
                                                ...
                                                <div className="flex flex-col mt-4">
                                                     ...
                                                     <div className="overflow-hidden rounded-xl border border-gray-200 dark:border-[#333] shadow-sm bg-white dark:bg-[#121212]">
                                                         {Array.isArray(problem?.expectedOutput) && problem.expectedOutput.length > 0 ? (
                                                             <div className="overflow-x-auto">
                                                                 <table className="w-full text-left text-xs whitespace-nowrap">
                                                                     <thead className="bg-slate-50 dark:bg-[#252525] border-b border-gray-200 dark:border-[#333]">
                                                                         <tr>
                                                                             {Object.keys(problem.expectedOutput[0]).map((f, j) => (
                                                                                 <th key={j} className="px-4 py-2.5 font-semibold text-slate-700 dark:text-gray-300">{f}</th>
                                                                             ))}
                                                                         </tr>
                                                                     </thead>
                                                                     <tbody className="divide-y divide-gray-100 dark:divide-[#333]">
                                                                         {problem.expectedOutput.map((r, rowIdx) => (
                                                                             <tr key={rowIdx} className="hover:bg-slate-50/80 dark:hover:bg-[#2a2a2a]">
                                                                                 {Object.keys(problem.expectedOutput[0]).map((f, colIdx) => (
                                                                                     <td key={colIdx} className="px-4 py-2.5 text-slate-600 dark:text-gray-400 font-mono">{String(r[f])}</td>
                                                                                 ))}
                                                                             </tr>
                                                                         ))}
                                                             ...
                                                             <div className="p-4 text-[13px] font-mono text-slate-600 dark:text-gray-400 whitespace-pre-wrap">
                                                                 {problem?.expectedOutput ? JSON.stringify(problem.expectedOutput, null, 2) : "Not available"}
                                                             </div>
                                                         )}
                                                     </div>
                                                </div>
                                        ...
                                        {consoleTab === 'output' && (
                                            !activeResult ? (
                                                <div className="flex flex-col items-center justify-center pt-12 text-slate-400">
                                                    ...
                                                    <span className="text-sm font-medium">Run Your Code To See Test Results</span>
                                                </div>
                                            ) : (
                                                <div className="space-y-8 max-w-4xl">
                                                    <div className="flex items-center gap-3">
                                                        <span className={`font-bold text-lg flex items-center gap-2 ${activeResult.passed ? 'text-emerald-600 dark:text-emerald-400' : 'text-red-600 dark:text-red-400'}`}>
                                                           {activeResult.passed ? <CheckCircle2 size={20}/> : <XCircle size={20}/>}
                                                           {activeResult.passed ? 'Accepted' : (activeResult.error ? 'Runtime Error' : 'Wrong Answer')}
                                                        </span>
                                                        <span className="text-xs font-mono px-2.5 py-1 box-border bg-slate-100 dark:bg-slate-800 rounded-md text-slate-500 uppercase tracking-wide">Time: {activeResult.timeMs}ms</span>
                                                    </div>
                                                    ...
                                                    {activeResult.isHidden ? (
                                                        <div className="bg-slate-50 dark:bg-[#1e1e1e] border border-gray-200 dark:border-[#333] p-6 rounded-xl flex flex-col items-center justify-center text-slate-500 font-medium text-sm">
                                                             For hidden test cases, expected and actual outputs are hidden.
                                                        </div>
                                                    ) : (
                                                        <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
                                                            {activeResult.userResult && (
                                                                <div className="flex flex-col">
                                                                <div className="text-xs font-semibold text-slate-700 dark:text-gray-400 mb-2 uppercase tracking-wide">Your Output</div>
                                                                            ...
                                                                            <thead className="bg-slate-50 dark:bg-[#252525] border-b border-gray-200 dark:border-[#333]">
                                                                                <tr>
                                                                                    {activeResult.userResult.fields.map((f, i) => (
                                                                                        <th key={"th-out-"+i} className="px-4 py-2.5 font-semibold text-slate-700 dark:text-gray-300">{f}</th>
                                                                                    ))}
                                                                                </tr>
                                                                            </thead>
                                                                            <tbody className="divide-y divide-gray-100 dark:divide-[#333]">
                                                                                {activeResult.userResult.rows.map((r, i) => (
                                                                                    <tr key={"tr-out-"+i} className="hover:bg-slate-50/80 dark:hover:bg-[#2a2a2a]">
                                                                                        {activeResult.userResult!.fields.map((f, j) => (
                                                                                            <td key={"td-out-"+j} className="px-4 py-2.5 text-slate-600 dark:text-gray-400 font-mono">{String(r[f])}</td>
                                                                                        ))}
                                                                                    </tr>
                                                                                ))}
                                                                                {activeResult.userResult.rows.length === 0 && (
                                                                                    <tr>
                                                                                        <td colSpan={activeResult.userResult.fields.length || 1} className="px-4 py-6 text-center text-slate-400 italic font-mono bg-slate-50/50 dark:bg-[#1a1a1a]">Empty result set</td>
                                                                                    </tr>
                                                                                )}
                                                                            </tbody>
                                                                        </table>
                                                        ...
                                                        {activeResult.expectedResult && !activeResult.error && (
                                                            <div className="flex flex-col">
                                                                <div className="text-xs font-semibold text-slate-700 dark:text-gray-400 mb-2 uppercase tracking-wide">Expected Output</div>
                                                                <div className="overflow-hidden rounded-xl border border-gray-200 dark:border-[#333] shadow-sm flex-1 bg-white dark:bg-[#121212]">
                                                                    <div className="overflow-x-auto">
                                                                        <table className="w-full text-left text-xs whitespace-nowrap">
                                                                            <thead className="bg-slate-50 dark:bg-[#252525] border-b border-gray-200 dark:border-[#333]">
                                                                                <tr>
                                                                                    {activeResult.expectedResult.fields.map((f, i) => (
                                                                                        <th key={"th-exp-"+i} className="px-4 py-2.5 font-semibold text-slate-700 dark:text-gray-300">{f}</th>
                                                                                    ))}
                                                                                </tr>
                                                                            </thead>
                                                                            <tbody className="divide-y divide-gray-100 dark:divide-[#333]">
                                                                                {activeResult.expectedResult.rows.map((r, i) => (
                                                                                    <tr key={"tr-exp-"+i} className={`${!activeResult.passed ? 'bg-red-50/40 dark:bg-red-900/10' : 'hover:bg-slate-50/80 dark:hover:bg-[#2a2a2a]'}`}>
                                                                                        {activeResult.expectedResult!.fields.map((f, j) => (
                                                                                            <td key={"td-exp-"+j} className="px-4 py-2.5 text-slate-600 dark:text-gray-400 font-mono">{String(r[f])}</td>
                                                                                        ))}
                                                                                    </tr>
                                                                                ))}
                                                                                {activeResult.expectedResult.rows.length === 0 && (
                                                                                    <tr>
                                                                                        <td colSpan={activeResult.expectedResult.fields.length || 1} className="px-4 py-6 text-center text-slate-400 italic font-mono bg-slate-50/50 dark:bg-[#1a1a1a]">Empty expected set</td>
                                                                                    </tr>
                                                                                )}
                                                                            </tbody>
                                                                        </table>
